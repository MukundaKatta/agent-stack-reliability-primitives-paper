...
| **Lens.org** (Search & Link) | ✗ skipped | ORCID OAuth succeeded, but Lens then required fresh account registration with username + email + use-type + T&C. Account creation is prohibited per safety rules. |
| **OpenAIRE Explore** (Search & Link) | ✗ skipped | ORCID OAuth succeeded, but OpenAIRE COmanage portal then required fresh Sign Up form with email + organisation + T&C accept (separate OpenAIRE-internal account). Account creation prohibited. Note: works still flow passively into OpenAIRE Graph via the DataCite → OpenAIRE pipeline, no action needed. |
| **BASE — Bielefeld Academic Search Engine** (Search & Link) | ⚠️ OAuth granted, 0 works indexed today | ORCID OAuth granted to BASE, but `aut:"Mukunda Katta"` query returned "No documents found" — BASE hasn't yet crawled the Zenodo deposits (typical lag 1–4 weeks). When BASE next crawls, the ORCID linkage is in place. Net: passive future. |
| **Zenodo Communities — Machine Learning** (slug `ml`) | ✅ **8 records submitted, Review requested** | All 8 unique Zenodo records submitted to the Zenodo "Machine Learning" community: agent-stack (20074702), Karna (20074229), Trajectory Replay v2 (20073574), Small-Rule Guardrails v2 (20057632), Chetana (20057058), ML Intern Lab (20057317), AI Eval Forge (20044318), Lightweight Evaluation Scorecards (20034550). Each shows "Review requested" — pending community curator approval. Curators of the ML community can view + edit metadata (CC-BY content, no risk). When approved, each record gets a "Part of: Machine Learning" badge and is discoverable via the community's Records tab. |
| **Zenodo Communities — Artificial intelligence and machine learning** | ✅ **8 records submitted, Review requested** | Same 8 records also submitted to the broader "Artificial intelligence and machine learning" Zenodo community (different community, different curators). 16 pending community reviews total across both communities. Doubles the inbound discovery surface within Zenodo. |
| **Zenodo Communities — Research Software Engineering** (slug `rse`, by Society of RSE) | ⚠️ **2 of 8 submitted** | agent-stack (20074702) and Karna (20074229) submitted to RSE community curated by the Society of Research Software Engineering. Stopping the third-community batch here as diminishing returns and the Zenodo dropdown UI started becoming unreliable for further automation. Remaining 6 records can be added manually in <2 minutes if desired. |
| **Internet Archive Wayback Machine** | ✅ **7 permanent snapshots created** | Save Page Now endpoints captured permanent archive URLs for the agent-stack public surfaces: landing page (`web.archive.org/web/20260508205955/`), ORCID profile, HuggingFace Hub model card, and 4 Zenodo records (Trajectory Replay v2 `20073574`, Guardrails v2 `20057632`, ML Intern Lab `20057317`, Lightweight Eval Scorecards `20034550`). These snapshots survive even if the live URLs go down, and are citable as `web.archive.org/web/<timestamp>/<url>` on any future paper or post. 5 URLs (GitHub repo + 4 Zenodo records) returned 520 from the unauthenticated Save endpoint and would need an authenticated Save to succeed; 3 URLs (Medium/dev.to/Hashnode posts) hit 429 rate limits and can be retried later. |
| **Software Heritage Save Code Now (round 2)** | ✅ **9 fresh save requests queued** | API-level `POST /api/1/origin/save/git/url/<repo>/` accepted 9 fresh visit requests (status `accepted/pending`) for these agent-stack repos: `agent-stack-reliability-primitives-paper`, `agent-stack`, `agentkit-py`, `agentkit`, `agenttrace`, `AgentBudgetMcp`, `AgentBudgetPy`, `agentcast-mcp`, `agentcast-py`. SH visits will create fresh snapshots that capture the latest commits. The other 16 repos (agentvet/agentsnap/agentguard/agentfit/AgentBudget families) hit a 429 hourly rate limit and can be re-submitted later from the user's IP. |
